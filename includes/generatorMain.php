...
class hssSkuGeneratorTools extends hssCommonTools {
  ...
  function hss_lm_enqueue_admin_js() {
    ...
    wp_register_style('hss-vuetify-style', SKU_GEN_PLUGIN_URL.'includes/css/vuetify.min.css');
    wp_enqueue_style('hss-roboto-font');
    wp_enqueue_style('hss-material-designs');
    wp_enqueue_style('hss-vuetify-style');

    if (SKU_GEN_ENVIRONMENT === 'development'  || (defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG )) {
      // Development or debugging load non-minified versions
      wp_enqueue_script( 'hss-vue-script', SKU_GEN_PLUGIN_URL.'includes/js/vue.js', array('jquery'));
      wp_enqueue_script( 'hss-vuetify-script', SKU_GEN_PLUGIN_URL.'includes/js/vuetify.js', array('jquery'));
    } else {
      //production environment load minified scripts
      wp_enqueue_script( 'hss-vue-script', SKU_GEN_PLUGIN_URL.'includes/js/vue.min.js', array('jquery'));
      wp_enqueue_script( 'hss-vuetify-script', SKU_GEN_PLUGIN_URL.'includes/js/vuetify.min.js', array('jquery'));
    }
    
    //TODO when minified versions of these files are made move them into the above if.
    wp_register_style( 'hss-sku-gen-css', SKU_GEN_PLUGIN_URL.'includes/css/hssSkuGenerator.css');
    wp_enqueue_style( 'hss-sku-gen-css' );
    wp_enqueue_script( 'hss-sku-gen-helper', SKU_GEN_PLUGIN_URL.'includes/js/hssSkuGenerator.js', array('jquery'));
    ...
    function hss_sku_gen_page() {
      ...
      ?>
          <div id="vueWrapper">
            <v-app v-cloak >
              <v-main>
                <input type="hidden" id="hssPtTodoJSVer" name="hssPtTodoJSVer" value="<?php echo SKU_GEN_JS_VERSION; ?>">
                <v-tabs v-model="tab">
                  <v-tab key="settings">Settings</v-tab>
                  <v-tab key="test">Test</v-tab>
                  <v-tab-item key="settings">
                    <?php include_once SKU_GEN_PLUGIN_PATH.'includes/tabs/settingsTab.php'; ?>
                  </v-tab-item>
                  <v-tab-item key="test">
                    <?php include_once SKU_GEN_PLUGIN_PATH.'includes/tabs/testTab.php'; ?>
                  </v-tab-item>
                </v-tabs>
              </v-main>
            </v-app>
